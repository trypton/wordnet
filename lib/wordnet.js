...
function lookup(word, callback) {
  ...
  onReady(function() {
    ...
    async.eachSeries(definitions, function(definition, callback) {

      /* Read the word data */
      readData(definition, true, function(err, result) {
...
function readDatabase(extension, callback) {
  ...
  reader.read(path.join(this.dataDir, 'index.' + extension), function(line) {
...
function WordNet(dataDir) {
  // Copy natural.WordNet approach
  if (!dataDir) {
    try {
      var WNdb = require('wordnet-db');
    } catch(e) {
      console.error("Please 'npm install wordnet-db' before using WordNet or specify a dict directory.");
      throw e;
    }
    dataDir = WNdb.path;
  }
  ...
  /* Read all files */
  async.eachSeries(_extensions, readDatabase.bind(this), function(err) {
    ...
    _extensions.forEach(function(extension) {
      _data[extensionToPos[extension]] = fs.openSync(path.join(dataDir, 'data.' + extension), 'r');
    });
