...
function parseDataLine(line) {
  ...
  meta.synsetType = synsetTypeMap[metadata.shift()];
  meta.wordCount = parseInt(metadata.shift(), 16);
  meta.words = [];

  /* Parse the words */
  for (let wordIdx = 0; wordIdx < meta.wordCount; wordIdx++) {
    meta.words.push({
  ...
  meta.pointers = [];
  for (let pointerIdx = 0; pointerIdx < meta.pointerCount; pointerIdx++) {
    meta.pointers.push({
      ...
      getData() {
        return getData(this).then((data) => data.shift());
      }
    });
  }
